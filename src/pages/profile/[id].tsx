...
export default function PlayerProfilePage() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      const allMatches = matchSnap.docs.map(doc => ({ id: doc.id, ...doc.data() }))
  ...
  return (
    <Layout>
      <h1 className="text-2xl font-bold mb-4">{player.displayName || player.email}</h1>
      ...
      <div className="space-y-3">
        {matches.map((match) => (
          <div key={match.id} className="border p-3 rounded bg-white shadow">
            <Link href={`/matches/${match.id}`} className="font-bold text-blue-600 hover:underline">
              Match vs {match.player1Id === id ? match.player2Name : match.player1Name}
            </Link>
